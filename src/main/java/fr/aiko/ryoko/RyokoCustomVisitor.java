...
public class RyokoCustomVisitor extends RyokoBaseVisitor {
    ...
    public Object visitSystemLib(RyokoParser.SystemLibContext ctx) {
        var mapUsed = variableMap;
        if (ctx.getParent().getParent().getParent() instanceof RyokoParser.FuncDeclarationContext) {
            String funcName = ((RyokoParser.FuncDeclarationContext) ctx.getParent().getParent().getParent()).IDENTIFIER().getText();
            mapUsed = functionVariableMap.get(funcName);
        }
        if (ctx.SHOW() != null) {
