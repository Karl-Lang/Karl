...
public final class Parser {
  ...
  private Statement funcCall() {
    ...
    return new FuncCallStatement(
        new FuncCallExpression(name, args, fileName, get(-2).getLine(), get(-2).getPosition()));
  }
  ...
  private Statement funcDeclaration() {
    ...
    if (ForbiddenNames.isForbiddenName(name)) {
      new RuntimeError(
          "Function name " + name + " is forbidden",
          fileName,
          get(-1).getLine(),
          get(-1).getPosition());
    }
  ...
  private Statement incrementDecrement() {
    ...
    if (operator.getType() == TokenType.PLUSPLUS) {
      ...
      return new IncrementDecrementStatement(
          name, TokenType.PLUS, fileName, nameToken.getLine(), nameToken.getPosition());
    } else {
      skip(TokenType.SEMICOLON);
      return new IncrementDecrementStatement(
          name, TokenType.MINUS, fileName, nameToken.getLine(), nameToken.getPosition());
    }
  }
      ...
      while (Operators.isOperator(getType())) {
        ...
        expression =
            new BinaryExpression(
                expression, right, operator, fileName, token.getLine(), token.getPosition());
      }
    }
      ...
      while (LogicalOperators.isOperator(getType())) {
        ...
        expression =
            new LogicalExpression(
                operator, expression, right, fileName, token.getLine(), token.getPosition());
      }
    }
  ...
  private Expression getValue() {
    if (getType() == TokenType.IDENTIFIER && get(1).getType() == TokenType.LEFT_PARENTHESIS) {
      ...
      return new FuncCallExpression(
          name, args, fileName, nameToken.getLine(), nameToken.getPosition());
      ...
      return new UnaryExpression(
          TokenType.EXCLAMATION, expr, fileName, get(-1).getLine(), get(-1).getPosition());
      ...
      return new VariableCallExpression(
          get(-1).getValue(), fileName, get(0).getLine(), get(0).getPosition());
    } else {
      new RuntimeError(
  ...
  private Statement variableAssignment() {
    ...
    return new VariableAssignmentStatement(
        name, expr, fileName, get(0).getLine(), get(0).getPosition());
  }
  ...
  private Statement variableDeclaration(boolean isFinal) {
    ...
    return new VariableDeclarationStatement(
        expression, name.getValue(), type, fileName, name.getLine(), name.getPosition(), isFinal);
  }
  ...
  private ShowStatement show() {
    ...
    while (!match(TokenType.RIGHT_PARENTHESIS)) {
      ...
      if (!match(TokenType.COMMA)
          && !checkType(0, TokenType.RIGHT_PARENTHESIS)
          && !checkType(1, TokenType.RIGHT_PARENTHESIS)) {
        new SyntaxError(
            "Excepted ',' for separate parameters",
            fileName,
            get(0).getLine(),
            get(0).getPosition());
      }
