...
public class Lexer {
  ...
  private void tokenizeChar() {
    ...
    if (input.charAt(position) != '\'') {
      if (input.charAt(position) != '\'') {
        new SyntaxError(
            "Character type can only contain one character", file.getStringPath(), line, position);
      } else {
  ...
  public void tokenizeNumber() {
    ...
    if (position + 1 < input.length() && Character.isLetter(input.charAt(position + 1))) {
      new SyntaxError(
          "Unexpected character: " + input.charAt(position), file.getStringPath(), line, position);
    }
    ...
    while (true) {
      if (c == '\\') {
        ...
        switch (c) {
          ...
          default -> new SyntaxError(
              "Invalid escape character: " + c, file.getStringPath(), line, position);
        }
        c = nextChar();
      }
  ...
  public void tokenizeOperator() {
    ...
    while (true) {
      ...
      if (OPERATORS.containsKey(buffer.toString())) {
        if (Character.toString(c).contentEquals(buffer)
